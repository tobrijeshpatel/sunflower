...
# ---- Auto Run Until Complete
if st.session_state.running and st.session_state.day < TOTAL_DAYS:
    advance_day()
    time.sleep(0.02)
    st.rerun()
...
# ---- Corner Labels with Blur + Shadow Effect
# Shadow layer
...
# ---- Plot Seeds with Fade-in Effect
x, y, colors, sizes = [], [], [], []
...
for n, r, age in st.session_state.seeds:
    ...
    fade = min(1.0, age / 10)
    colors.append((1 - fade, 0.5 * fade, 0.1))
    sizes.append(10 + fade * 6)
